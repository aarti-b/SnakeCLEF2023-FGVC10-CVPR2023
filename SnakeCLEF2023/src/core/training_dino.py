...
from torch.optim import SGD, Adam, lr_scheduler, NAdam, AdamW, Adagrad
...
def train_epoch(base_model, model, dataloader, criterion, optimizer, *, accumulation_steps=1,
    ...
    for i, (imgs, targs) in pbar(enumerate(dataloader), total=len(dataloader)):
        ...
        with torch.no_grad():
            features=base_model.forward_features(imgs)['x_norm_clstoken']
...
def predict(base_model,model, dataloader, criterion=None, *, device=None, pbar=None):
    if device is None:
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    if pbar is None:
        pbar = ProgressBar().progress_bar
...
class Trainer:
    def __init__(self, base_model,model, trainloader, criterion, opt_fn=None, sched_fn=None, *,
        ...
        self.compute_scores_fn = classification_scores
    ...
    def train_epoch(self, optimizer, scheduler=None, pbar=None, *, return_preds=False):
        return train_epoch(
            self.base_model,self.model, self.trainloader, self.criterion, optimizer, scheduler=scheduler, 
    ...
    def predict(self, dataloader, pbar=None):
        return predict(
            self.base_model,self.model, dataloader, self.criterion,
